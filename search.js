...
class Search {
  ...
  search() {
    ...
    return fetch(`https://api.foursquare.com/v2/venues/search?ll=${loadedMap.position.lat},${loadedMap.position.lng}&query=${this.searchTerm}&oauth_token=${foursquareKey}`)
  ...
  createVenues(venues) {
    ...
    }).then(loadedMap.plotVenues()).then(this.addSearchResults());
  }
}
